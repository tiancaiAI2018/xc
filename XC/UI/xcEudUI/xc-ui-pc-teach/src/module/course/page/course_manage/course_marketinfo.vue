  ...
  export default {
    ...
    mounted(){
      ...
      systemApi.sys_getDictionary('203').then((res) => {
        this.chargeList = res.sysDictionary.dvalue;
      });
      systemApi.sys_getDictionary('204').then((res) => {
        this.validList = res.sysDictionary.dvalue;
      });

      //获取课程营销信息
      courseApi.getCourseMarketById(this.courseid).then((res) => {
         //console.log(res);
        if(res.courseMarket && res.courseMarket.id){
           this.courseMarketForm = res.courseMarket;
        }
      });
    }
  }
