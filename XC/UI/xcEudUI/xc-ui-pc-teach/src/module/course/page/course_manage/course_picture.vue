...
//        alert('上传成功')
        ...
        if(response.success){
          ...
          courseApi.addCoursePic(this.courseid,{"courseid":this.courseid,"pic":fileId}).then(res=>{
              if(res.success){
    ...
    mounted(){
      ...
      courseApi.findCoursePicList(this.courseid).then(res=>{
          if(res.coursePic && res.coursePic.pic){
              let imgUrl = this.imgUrl+res.coursePic.pic;
              //将图片地址设置到
            this.fileList.push({name:'pic',url:imgUrl,fileId:res.coursePic.pic})
          }
